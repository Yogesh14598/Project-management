...
import ProfileusersDetail from "./ProfileusersDetail";
...
const ProfileEdit = (props) => {
  ...
  // if (res) { 
  //   renderContactList  = res.map((event) => { 
  //         return (
  //           <div>
  //            <p>{event.firstname}</p>
  //           </div>
  ...
  const valupdate = () => {
    props.user(firstname,lastname,email,dropdown);
  }
  return (
    <div className="">
...
<form>
            ...
            <Link to="/ProfileusersDetail" className="side_links">
              <Button type="button" text="Update" onclick={valupdate} />
            </Link>
</form>
