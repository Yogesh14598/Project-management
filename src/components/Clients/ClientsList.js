...
const ClientsList = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${BASE_URL}/admin/project-management/potential-projects`,
        {
          headers: {
  ...
  const restoreHandler = (id) => {
    ...
    if (todelete) {
      axios
        .get(
          `${BASE_URL}/admin/project-management/projects/${id}/restore`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              "Access-Control-Allow-Origin": "*",
  ...
  const Showsearchnbtn = () => (
    <div className="">
      <img src="/images/Icon awesome-filter.svg" class="mx-3 mb-2" />
      <input
        type="text"
        placeholder="Search"
        autoFocus
