...
const NotesLists = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${BASE_URL}/admin/project-management/projects/${projectId}/discussions`,
        {
          headers: {
  ...
  const deleteHandler = (id) => {
    ...
    if (todelete) {
      axios
        .delete(`${BASE_URL}/admin/project-management/discussions/${id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
            "Access-Control-Allow-Origin": "*",
            "Access-Control-Allow-Methods": "GET,PUT,POST,DELETE,PATCH,OPTIONS",
  ...
  const editHandler = (id) => {
    axios
      .get(`${BASE_URL}/admin/project-management/discussions/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
          "Access-Control-Allow-Origin": "*",
          "Access-Control-Allow-Methods": "GET,PUT,POST,DELETE,PATCH,OPTIONS",
      ...
      .then((response) => {
        ...
        localStorage.setItem(
          "discussion-head",
          response.data.discussion.heading
        );
        localStorage.setItem(
          "discussion-desc",
          response.data.discussion.description
        );
        ...
        <div>
          {loading ? (
            ...
            discussions.map((item) =>
              item === "" ? (
                <Message>No Discussuions to Display</Message>
              ) : (
                <div key={item.id} className="projectmaintabs_update_sec">
                  <div className="projectmaintabs_update_section">
                    ...
                    <p style={{ fontWeight: "600" }}>
                      {dateFormat(item.created_at, "mmm d, yyyy")}
                    </p>
                  </div>
