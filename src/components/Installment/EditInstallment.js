...
const EditInstallment = ({ history }) => {
  ...
  let ipaiddate = localStorage.getItem("installment-paiddate");
  let ideadlinedate = localStorage.getItem("installment-deadlinedate");
  const ipaiddates = moment(new Date(ipaiddate)).format(
    "YYYY-MM-DD"
  );
  const ideadlinedates = moment(new Date(ideadlinedate)).format(
    "YYYY-MM-DD"
  );
  ...
  useEffect(() => {
    if ((installmentName, installmentnumber, amount, description == "")) {
      ...
      setPaidDate(ipaiddates);
      setDeadline(ideadlinedates);
    } else {
      console.log("details are filled");
    }
  }, []);
  console.log(ideadlinedates ,"dateval" );
  console.log(ipaiddates, "datevaslues");
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .put(
        `${BASE_URL}/admin/project-management/installments/${id}`,
